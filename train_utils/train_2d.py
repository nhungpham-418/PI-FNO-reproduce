...
from .losses import LpLoss, darcy_loss, PINO_loss, traffic_loss, advection_loss
...
#############################################################################################
# New function to train Burgers Equation with mask


def train_2d_burger_mask(model,
                    train_loader, v, mask, 
                    optimizer, scheduler,
                    config,
                    rank=0, log=False,
    ...
    for e in pbar:
        ...
        for x, y in train_loader:
            ...
            mask = mask.to(rank)
            out = model(x).reshape(y.shape)
            data_loss = myloss(out, y, mask)

            orig_data_loss = myloss(out, y)
            ...
            data_l2 += data_loss.item()
            # Track all data loss
            data_all += orig_data_loss.item()
            #########################
            train_pino += loss_f.item()
            train_loss += total_loss.item()
        scheduler.step()
        data_l2 /= len(train_loader)
        data_all /= len(train_loader) 
        train_pino /= len(train_loader)
        train_loss /= len(train_loader)
        if use_tqdm:
            pbar.set_description(
...
#############################################################################################
# New function to train Traffic Equation

def train_2d_traffic(model,
                    train_loader, v,
                    optimizer, scheduler,
                    config,
                    rank=0, log=False,
    ...
    for e in pbar:
        ...
        for x, y in train_loader:
            ...
            loss_u, loss_f = traffic_loss(out, x[:, 0, :, 0], v)
            total_loss = loss_u * ic_weight + loss_f * f_weight + data_loss * data_weight
...
#############################################################################################
# New function to train Advection

def train_2d_advection(model,
                    train_loader,
                    optimizer, scheduler,
                    config,
                    rank=0, log=False,
    ...
    for e in pbar:
        ...
        for x, y in train_loader:
            ...
            loss_u, loss_f = advection_loss(out, x[:, 0, :, 0])
            total_loss = loss_u * ic_weight + loss_f * f_weight + data_loss * data_weight
