...
class LpLoss(object):
    ...
    def abs(self, x, y, mask=None):
        ...
        if mask is not None:
            ...
            # Apply mask: zero out non-masked points, keeping only masked ones
            x = x * mask
            y = y * mask
...
#############################################################################################
# New loss for Advection Equation
def FDM_Advection(u, D=1):
    batchsize = u.size(0)
    nt = u.size(1)
    nx = u.size(2)
    ...
    # Calculate the nonlinear term
    nonlinear_term = 2 * t_mid * ux_mid - torch.cos(t_mid) * u_mid

    # 5. Compute Residual
    # Equation: u_t + t(u - u^2/2)u_x = 0
    Du = ut + nonlinear_term
    
    return Du

def advection_loss(u, u0):
    batchsize = u.size(0)
    nt = u.size(1)
    nx = u.size(2)
