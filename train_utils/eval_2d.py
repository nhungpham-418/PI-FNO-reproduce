...
###################################################################################
# Eval function for traffic
def eval_traffic(model,
                 dataloader,
                 v,
                 config,
                 device,
    ...
    for x, y in pbar:
        ...
        loss_u, f_loss = traffic_loss(out, x[:, 0, :, 0], v)
        test_err.append(data_loss.item())
        f_err.append(f_loss.item())
...
###################################################################################
# Eval function for advection
def eval_advection(model,
                 dataloader,
                 config,
                 device,
                 use_tqdm=True):
    ...
    for x, y in pbar:
        ...
        loss_u, f_loss = advection_loss(out, x[:, 0, :, 0], v)
        test_err.append(data_loss.item())
        f_err.append(f_loss.item())
